...
const frameHistory = game?.frameHistory || []
    
const frameScores = game?.frameScores || []

const splitFrames = game?.splitFrames || []

const cleanFrames = frameHistory.filter(frame => frame.includes("X") ||
       frame.includes("/")
    ).length
...
const splitCount = splitFrames.length

const convertedSplits = splitFrames.filter(
    split => split.converted
).length

const splitPct = splitCount === 0
? 0
:Math.round ((convertedSplits/splitCount) * 100)
...
<>
        <div className="bg-[#f8f7f5] border border-[#C9B07A] rounded-2xl p-6 mt-6 gap-6 flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-4 gap-x-48 gap-y-4 max-w-md mx-auto">
                ...
                <div><p className = "text-slate-700 font-bold">Date: {game?.date ? new Date(game?.date).toLocaleDateString()
                : "No Date"}</p></div>
            </div>
            ...
            <div className="flex justify-center gap-2">
                ...
                <div className="flex items-center mt-8 mb-6">
                <div className = "bg-[#F8F7F5] border border-[#C9B07A] rounder-xl p-4 mb-6">
                    <h2 className="font-bold text-xl mb-2">
                        Statistics
                    </h2>
                    <p className="text-slate-700">Splits: {splitCount}</p>
                    <p className="text-slate-700">Split Conversion: {splitPct}%</p>
                    <p className="text-slate-700">Clean Frames: {cleanFrames}/10 ({cleanPct}%)</p>
                </div>
